...
rhdz::rhdz(QString clientName) : _midiLearner(this)
{
  _menu=NULL;
...
void rhdz::setUpGui() {
  setWindowTitle(QString("[%1] rhdz").arg(_clientName));
  ...
  _staxaudio.setPixmap(QPixmap(":/rhdz"));
  ...
  _mixerGrid.horizontalHeader()->setDefaultSectionSize(60);
  _mixerGrid.verticalHeader()->setDefaultSectionSize(324);
...
void rhdz::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void rhdz::addMixerClicked()
{
  ...
  if (ok && !name.isEmpty()) {
    ...
    if(_mixerGrid.columnCount()>8){
         _mixerGrid.setCellWidget(1, _mixerGrid.columnCount() - 11, vm);
    }
    else{
...
void rhdz::removeMixerClicked()
{
  QStringList items;
  int i;
  qv *vm;
  for (i=0; i < _mixerGrid.columnCount(); i++) {
    vm=(qv*)_mixerGrid.cellWidget(0, i);
    items << vm->getID();
  }
  bool ok;
...
void rhdz::handleMidiEvent(struct MidiEvent ev)
{
...
bool rhdz::saveConf()
{
  ...
  for (int i=0; i < _mixerGrid.columnCount(); i++) {
    ...
    qv *vm=(qv*)_mixerGrid.cellWidget(0, i);
...
bool rhdz::loadConf()
{
  ...
  if (mixers.length()) {
    ...
    for (uint i=0; i < mixers.length(); i++) {
      ...
      qv *vm=new qv(mixer.attribute("id"));
      if (!_jack.addMixer(vm)) {
	delete vm;
      }
      else {
